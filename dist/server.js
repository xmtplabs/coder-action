...
var require_thread_stream = __commonJS((exports, module) => {
  var __dirname = "/workspaces/coder-action/node_modules/thread-stream";
...
var require_transport = __commonJS((exports, module) => {
  var __dirname = "/workspaces/coder-action/node_modules/pino/lib";
  var { createRequire } = __require("module");
  var { existsSync } = __require("fs");
  var getCallers = require_caller();
  var { join, isAbsolute, sep: sep2 } = __require("path");
...
var require_tools = __commonJS((exports, module) => {
  ...
  function noop3() {}
  function genLog(level, hook6) {
    if (!hook6)
      return LOG;
    return function hookWrappedLog(...args) {
      hook6.call(this, args, LOG, level);
    };
    function LOG(o, ...n) {
      if (typeof o === "object") {
        let msg = o;
...
var require_levels = __commonJS((exports, module) => {
  ...
  var levelMethods = {
    fatal: (hook6) => {
      const logFatal = genLog(DEFAULT_LEVELS.fatal, hook6);
    ...
    error: (hook6) => genLog(DEFAULT_LEVELS.error, hook6),
    warn: (hook6) => genLog(DEFAULT_LEVELS.warn, hook6),
    info: (hook6) => genLog(DEFAULT_LEVELS.info, hook6),
    debug: (hook6) => genLog(DEFAULT_LEVELS.debug, hook6),
    trace: (hook6) => genLog(DEFAULT_LEVELS.trace, hook6)
  };
  ...
  function setLevel(level) {
    ...
    const hook6 = this[hooksSym].logMethod;
    for (const key in values) {
      if (levelComparison(values[key], levelVal) === false) {
        this[key] = noop3;
        continue;
      }
      this[key] = isStandardLevel(key, useOnlyCustomLevelsVal) ? levelMethods[key](hook6) : genLog(values[key], hook6);
    }
...
function Collection() {
  ...
  const hook5 = register.bind(null, state);
  bindApi(hook5, state);
  return hook5;
}
...
var AppConfigSchema = exports_external.object({
  ...
  privateKey: exports_external.string().min(1),
  webhookSecret: exports_external.string().min(1),
  agentGithubUsername: exports_external.string().min(1).default("xmtp-coder-agent"),
  ...
  coderTemplateName: exports_external.string().min(1).default("task-template"),
  coderTemplatePreset: exports_external.string().min(1).optional(),
  coderOrganization: exports_external.string().min(1).default("default"),
  logFormat: exports_external.string().optional(),
...
function createLogger2(options) {
  ...
  const pinoLogger = useJson ? import_pino.default() : import_pino.default({
    transport: {
      target: "pino-pretty",
...
// src/coder-client.ts
var TaskIdSchema = exports_external.string().uuid().brand("TaskId");
var TaskNameSchema = exports_external.string().min(1).brand("TaskName");
...
var CoderSDKTemplateSchema = exports_external.object({
  id: exports_external.string().uuid(),
  name: exports_external.string(),
  description: exports_external.string().optional(),
  organization_id: exports_external.string().uuid(),
  active_version_id: exports_external.string().uuid()
});
var CoderSDKTemplateVersionPresetSchema = exports_external.object({
  ID: exports_external.string().uuid(),
  Name: exports_external.string(),
...
var ExperimentalCoderSDKCreateTaskRequestSchema = exports_external.object({
  ...
  template_version_preset_id: exports_external.string().min(1).optional(),
  input: exports_external.string().min(1)
});
var ExperimentalCoderSDKTaskStateEntrySchema = exports_external.object({
  state: exports_external.enum(["idle", "working", "complete", "failed"])
});
var ExperimentalCoderSDKTaskStatusSchema = exports_external.enum([
  "pending",
  "initializing",
  "active",
  "paused",
...
var ExperimentalCoderSDKTaskSchema = exports_external.object({
  id: TaskIdSchema,
  name: TaskNameSchema,
  owner_id: exports_external.string().uuid(),
  template_id: exports_external.string().uuid(),
  workspace_id: exports_external.string().uuid(),
...
var WorkspaceSchema = exports_external.object({
  ...
  latest_build: exports_external.object({
    status: exports_external.string(),
    transition: exports_external.string()
  })
});
...
class RealCoderClient {
  ...
  async request(endpoint2, options) {
    const url = `${this.serverURL}${endpoint2}`;
  ...
  async getCoderUserByGitHubId(githubUserId) {
    ...
    const endpoint2 = `/api/v2/users?q=${encodeURIComponent(`github_com_user_id:${githubUserId}`)}`;
  ...
  async getTemplateByOrganizationAndName(organizationName, templateName) {
    const endpoint2 = `/api/v2/organizations/${encodeURIComponent(organizationName)}/templates/${encodeURIComponent(templateName)}`;
    const response = await this.request(endpoint2);
    return CoderSDKTemplateSchema.parse(response);
  }
  async getTemplateVersionPresets(templateVersionId) {
    const endpoint2 = `/api/v2/templateversions/${encodeURIComponent(templateVersionId)}/presets`;
    const response = await this.request(endpoint2);
  ...
  async getTaskById(owner, taskId) {
    const endpoint2 = `/api/experimental/tasks/${encodeURIComponent(owner)}/${encodeURIComponent(taskId)}`;
    const response = await this.request(endpoint2);
    return ExperimentalCoderSDKTaskSchema.parse(response);
  }
  async createTask(owner, params) {
    const endpoint2 = `/api/experimental/tasks/${encodeURIComponent(owner)}`;
    const response = await this.request(endpoint2, {
      method: "POST",
...
// src/webhook-schemas.ts
var WebhookInstallationSchema = exports_external.object({
  id: exports_external.number()
}).passthrough();
var WebhookRepositorySchema = exports_external.object({
  name: exports_external.string(),
  owner: exports_external.object({
    login: exports_external.string()
  }).passthrough(),
  full_name: exports_external.string()
}).passthrough();
var WebhookSenderSchema = exports_external.object({
  id: exports_external.number(),
  login: exports_external.string()
}).passthrough();
var IssuesAssignedPayloadSchema = exports_external.object({
  action: exports_external.literal("assigned"),
  assignee: exports_external.object({
    login: exports_external.string(),
    id: exports_external.number()
  }).passthrough(),
  issue: exports_external.object({
...
var IssuesClosedPayloadSchema = exports_external.object({
  action: exports_external.literal("closed"),
  issue: exports_external.object({
    number: exports_external.number()
  }).passthrough(),
  repository: WebhookRepositorySchema,
...
var IssueCommentCreatedPayloadSchema = exports_external.object({
  action: exports_external.literal("created"),
  issue: exports_external.object({
    number: exports_external.number(),
    pull_request: exports_external.object({
...
var PRReviewCommentCreatedPayloadSchema = exports_external.object({
  action: exports_external.literal("created"),
  pull_request: exports_external.object({
    number: exports_external.number(),
    user: exports_external.object({
...
var PRReviewSubmittedPayloadSchema = exports_external.object({
  action: exports_external.literal("submitted"),
  pull_request: exports_external.object({
    number: exports_external.number(),
    user: exports_external.object({
      login: exports_external.string()
  ...
  review: exports_external.object({
    ...
    html_url: exports_external.string(),
    submitted_at: exports_external.string(),
    user: exports_external.object({
      login: exports_external.string()
    }).passthrough()
  }).passthrough(),
...
var WorkflowRunCompletedPayloadSchema = exports_external.object({
  action: exports_external.literal("completed"),
  workflow_run: exports_external.object({
    id: exports_external.number(),
    name: exports_external.string(),
    path: exports_external.string().nullable().optional(),
    head_sha: exports_external.string(),
    html_url: exports_external.string(),
    conclusion: exports_external.string().nullable(),
    pull_requests: exports_external.array(exports_external.object({
      number: exports_external.number()
    }).passthrough())
  }).passthrough(),
  repository: WebhookRepositorySchema,
...
class WebhookRouter {
  ...
  async handleWebhook(eventName, deliveryId, payload) {
...
`);
  ...
  async addReactionToComment(owner, repo, commentId) {
    try {
      ...
      this.logger.warn(`Failed to add reaction to comment ${commentId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  async addReactionToReviewComment(owner, repo, commentId) {
    try {
      ...
      this.logger.warn(`Failed to add reaction to review comment ${commentId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
class PRCommentHandler {
  ...
  async run() {
    ...
    if (linkedIssues.length > 1) {
      this.logger.warn(`Multiple linked issues found, using first: #${linkedIssues[0].number}`);
    }
    const issue = linkedIssues[0];
    const taskName = generateTaskName(this.inputs.coderTaskNamePrefix, this.context.repo, issue.number);
    const task = await lookupAndEnsureActiveTask(this.coder, this.inputs.coderUsername, taskName, this.logger);
...
class HandlerDispatcher {
  ...
  async dispatch(result, requestLogger) {
    const logger = requestLogger ?? this.options.logger;
    const octokit = this.options.createInstallationOctokit(result.installationId);
    const createGitHubClient = this.options.createGitHubClient ?? ((oct) => new GitHubClient(oct, logger));
  ...
  return (context, next) => {
    ...
    async function dispatch(i) {
      ...
      if (middleware[i]) {
        ...
        handler2 = i === middleware.length && next || undefined;
      }
      if (handler2) {
        try {
  ...
  add(method, path, handler2) {
    ...
    if (/\*$/.test(path)) {
      ...
      Object.keys(routes).forEach((m) => {
        if (method === METHOD_NAME_ALL || method === m) {
          Object.keys(routes[m]).forEach((p) => re.test(p) && routes[m][p].push([handler2, paramCount]));
        }
      });
      return;
    }
...
var PreparedRegExpRouter = class {
  ...
  #addPath(method, path, handler2, indexes, map) {
    const matcher = this.#matchers[method];
    if (!map) {
      matcher[2][path][0].push([handler2, {}]);
    } else {
      indexes.forEach((index) => {
        if (typeof index === "number") {
          matcher[1][index].push([handler2, map]);
        } else {
          matcher[2][index || path][0].push([handler2, map]);
        }
      });
    }
  }
  add(method, path, handler2) {
    ...
    for (const [indexes, map] of data) {
      if (method === METHOD_NAME_ALL) {
        for (const m in this.#matchers) {
          this.#addPath(m, path, handler2, indexes, map);
        }
      } else {
        this.#addPath(method, path, handler2, indexes, map);
      }
    }
  }
  buildAllMatchers() {
...
function createApp(options) {
  ...
  app.post("/api/webhooks", async (c) => {
    ...
    let handleResult2;
    try {
      handleResult2 = await handleWebhook(eventName, deliveryId, payload, reqLogger);
    } catch (err) {
...
// src/main.ts
async function createStartupContext(options) {
...
async function main() {
  ...
  const logger = createLogger2({ logFormat: config.logFormat });
  const appOctokit = new Octokit2({
